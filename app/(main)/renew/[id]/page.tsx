...
export default function RenewPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/subscriptions/${id}/renew`)
      ...
      .then((json) => {
        ...
        // Default amountPaid to full amount for 1 month
  ...
  if (success) {
    return (
      <div className="max-w-lg mx-auto mt-16 text-center space-y-4 animate-in fade-in duration-300">
        <div className="w-16 h-16 rounded-full bg-emerald-500/10 text-emerald-600 flex items-center justify-center mx-auto">
          <CheckCircle2 size={28} />
        </div>
        <h2 className="text-xl font-bold text-foreground">Renewed!</h2>
        <p className="text-sm text-muted-foreground">
          Subscription renewed for {months} month(s) starting{" "}
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="space-y-3">
          ...
          <p className="text-base text-muted-foreground">
            <span className="font-semibold text-foreground">Seat {data.seatNo}</span> • {data.floorName}
          </p>
        </div>
